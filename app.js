(function () {
  ...
  const APP_RE =
    /^appendix[\s_\-]*([0-9]+)(?:[^\d]*(\d+))?(?:.*?\((\d+)\s*of\s*(\d+)\))?/i;
  ...
  function sortIntoGroups(files) {
    ...
    for (const f of files) {
      ...
      if (!key) {
        log(`WARN: skipping (bad name): ${f.name}`);
        continue;
      }
    ...
    items.sort((a, b) => {
      if (a.key.appendix_num !== b.key.appendix_num)
        return a.key.appendix_num - b.key.appendix_num;
      const ay = a.key.part_y,
        by = b.key.part_y;
  ...
  // NEW: unified label builder -> "<filename-without-ext> – <header>" (header optional)
  function buildHeaderLabel(nameWithExt, header, font) {
    const base = nameWithExt.replace(/\.[^/.]+$/, "");
    const label = header && header.trim().length > 0 ? `${base} – ${header}` : base;
    return { label, widthAt: (size) => font.widthOfTextAtSize(label, size) };
  }
  ...
  async function run() {
    ...
    async function finalize() {
      ...
      // re-embed font in new document
      const reFont = await doc.embedFont(StandardFonts.Helvetica);
      // overwrite reference
      Object.assign(font, reFont);
      curPages = 0;
      batchStartPage = 1;
    }
    ...
    for (const group of groups) {
      ...
      for (const it of group.items) {
        // Build the header label once per file
        const headerLabel = buildHeaderLabel(it.name, header, font);
        ...
        if (it.isPDF) {
          ...
          for (let i = 0; i < total; i++) {
            ...
            const textWidth = headerLabel.widthAt(FONT_SIZE);
            page.drawText(headerLabel.label, {
              x: right - textWidth,
              y: LETTER_H - MARGIN - FONT_SIZE - 4,
              size: FONT_SIZE,
              font,
              color: rgb(0, 0, 0),
            });
